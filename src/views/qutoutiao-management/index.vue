<template>
  <div class="app-container">
    ...
    <div v-if="device=='desktop'" class="filter-container">
      ...
      <el-select size="mini" v-model="accountValue" clearable filterable placeholder="账户">
        <el-option
    ...
    >
      ...
      <el-table-column label="套餐属性" width="150px" sortable="custom" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.packageAttributes }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="核单间隔" width="120px" align="center">
        <template slot-scope="scope">
    ...
    <div v-if="device=='mobile'" class="search-container">
      <van-popup v-model="mobileSearchShow" position="right">
        <div style="height:100vh;width:100vw;">
          ...
          <van-cell-group>
            ...
            <van-cell
              @click="handleChoosePackageAttributes"
              title="套餐属性"
              is-link
              :value="packageAttributesMobileValue"
            />
            <van-cell
      ...
      <!--套餐属性选择弹窗-->
      <van-popup v-model="mobilePackageAttributesPickerShow" position="bottom">
        <van-search
          clearable
          show-action
          placeholder="请输入搜索关键词"
          v-model="PackageAttributesSearchValue"
          list="packageAttributesIde"
        >
          <div slot="action" @click="handleSearchPackageAttributesItem">搜索</div>
        </van-search>
        <datalist id="packageAttributesIde">
          <option v-for="item in packageOptions" :key="item.value" :value="item.label"/>
        </datalist>
        <van-picker
          show-toolbar
          :default-index="packageAttributesPickerCurrentSelect"
          :columns="packageColumns"
          @cancel="mobilePackageAttributesPickerShow = false"
          @confirm="PackageAttributesPickerConfirm"
        />
      </van-popup>
      <!--是否有效选择弹窗-->
...
export default {
  data() {
    return {
      ...
      accountColumns: ["账户1", "账户2", "账户3", "账户4"],
      productOptions: [
        {
          value: "选项1",
          label: "产品1"
      ...
      productColumns: ["产品1", "产品2", "产品3", "产品4", "产品5"],
      packageOptions: [
        {
          value: "选项1",
          label: "套餐1"
      ...
      packageColumns: ["套餐1", "套餐2", "套餐3", "套餐4", "套餐5"],
      usefulOptions: [
        {
          value: "选项1",
          label: "是"
  ...
  methods: {
    ...
    //选择套餐属性
    handleChoosePackageAttributes() {
      if (!this.mobilePackageAttributesPickerShow) {
        this.mobilePackageAttributesPickerShow = !this
          .mobilePackageAttributesPickerShow;
      }
    },
    //选择是否有效
    ...
    //确认选择套餐属性
    PackageAttributesPickerConfirm(res) {
      if (this.mobilePackageAttributesPickerShow) {
        this.packageAttributesMobileValue = res;
        this.mobilePackageAttributesPickerShow = !this
          .mobilePackageAttributesPickerShow;
      }
    },
    ...
    //套餐属性快捷搜索
    handleSearchPackageAttributesItem() {
      this.packageAttributesPickerCurrentSelect = this.packageColumns.indexOf(
        this.PackageAttributesSearchValue
      );
    },
    //开始搜索
    handleMobileSearch() {
      ...
      console.log("账户" + this.accountValue);
      console.log("业务员:" + this.salesmanMobileValue);
      console.log("套餐属性" + this.packageAttributesValue);
